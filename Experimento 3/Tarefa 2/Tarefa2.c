...
void Consumidor( int count ){
    ...
    while(1) {
        ...
        */
        ...
        tmp_index_consumidor = *g_shm_addr_consumidor;
         ...
         /*
                 ...
                 */
                for( i = 0; i < number; i++ ) {
                    if( ! (tmp_index_consumidor + i > sizeof(tmp_index_buffer)) ) {
                         if(! (tmp_index_buffer + i > MAX_SIZE_BUFFER)){
                             //Verifica se buffer esta cheio
                             //Coloca no buffer e atualiza índice
                             buffer[tmp_index_buffer + i] = '#';
                        usleep(1);
                         }
                    }
                }
        ...
        *g_shm_addr_produtor   = limite_buffer + i;
        *g_shm_addr_buffer = tmp_index_buffer + i;
        ...
        // Printa o conteudo que cada filho produziu
        fprintf(stderr, "\nCaracteres consumidos pelo filho %i\n", count);
        int j;
